...
# --- 4. 點名與統計 ---
    ...
    st.title("📝 點名紀錄與出席率統計")
    st.data_editor(st.session_state.players_df[["姓名", "年級", "班級", "出席率"]], use_container_width=True, key="attendance_viewer")
        ...
        for idx, row in df.iterrows():
            with cols[idx % 2]:
                with st.container(border=True):
                    ...
                    if row.get('狀態') == "接受報名":
                        st.button(f"🔗 查看報名連結", key=f"btn_ev_{idx}")
    else:
        st.info("目前沒有進行中的活動。")
